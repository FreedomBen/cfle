...
#  sed -i -E -e "s|image: docker.io/freedomben/cfle-(.*):.*|image: docker.io/freedomben/cfle-\1:${NEW_VERSION}|g" "$f"
#done

for f in ${K8S_FILES[@]}; do
  sed -i -E -e "s|image: docker.io/freedomben/cfle(.*):.*|image: docker.io/freedomben/cfle:${NEW_VERSION}|g" "$f"
done
